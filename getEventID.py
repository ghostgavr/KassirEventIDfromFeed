...
with open('test.csv', 'w', newline="") as fileprint: #encoding='utf-8'
    ...
    writer.writerow(["EventID", "Name", "Venue", "VenueID", "URL"])  # write header
    ...
    for x in parsed:
        ...
        if parsed[x]['venue'] not in halldict:
            ...
            writer.writerow([str(x), str(parsed[x]['name']), halldict[parsed[x]['venue']], str(parsed[x]['venue']), str(parsed[x]['url'])])
